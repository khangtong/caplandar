// Code for converting solar date to lunar date
/*
...
function jdFromDate(dd: number, mm: number, yy: number): number {
  if (!Number.isInteger(dd) || !Number.isInteger(mm) || !Number.isInteger(yy)) {
    throw new Error('All date parameters must be integers');
  }
  var a, y, m, jd;
  a = INT((14 - mm) / 12);
  y = yy + 4800 - a;
...
export function convertLunar2Solar(
  ...
  if (lunarDay < 1 || lunarDay > 30 || lunarMonth < 1 || lunarMonth > 12) {
    throw new Error('Invalid lunar date values');
  }
  ...
  if (b11 - a11 > 365) {
    ...
    if (lunarLeap != 0 && lunarMonth != leapMonth) {
      throw new Error('Invalid lunar leap month combination');
